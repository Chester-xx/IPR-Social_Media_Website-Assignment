...
include_once("../../includes/functions.php");
    ...
    try {
        ...
        if ($uid === 0) {
            throw new Exception("User ID not specified - Bad request", 400);
        }
        // how many posts we will send at once
        $loadlimit = 10;
        // the position of posts in the db
        $offset = isset($_GET["offset"]) ? intval($_GET["offset"]) : 0;
        // Compared to request.php, here we specify p.UserID to only collect the specifics
        $stmt = $conn->prepare("Select u.Username, u.PFP, p.Content, p.Image, p.CreateTime From tblPosts p Join tblUsers u On p.UserID = u.UserID Where ? = p.UserID Order By p.PostID Desc Limit ? Offset ?");
        // failed to prep qry
        if (!$stmt) throw new Exception("Connection preperation failed: " . $conn->error);
        $stmt->bind_param("iii", $uid, $loadlimit, $offset);
